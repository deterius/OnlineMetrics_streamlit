...
# -------------- Import and process
    df = pd.read_csv('combineddata/Flip_order_data/ALLorders.csv')
    ...
    df.to_csv('combineddata/Flip_order_data/FlipOrders_combined.csv')
#---------------------------------------

    # -----------SET OPTIONS-----------
    freq = df[['Date','Store Name','Time','Day','Revenue','Channel']]
    ...
    st.sidebar.header("Time Duration")
    date_start = st.sidebar.date_input('start date', freq['Date'].min())
    date_end = st.sidebar.date_input('end date', freq['Date'].max())

    if date_end > freq['Date'].max():
        date_end = freq['Date'].max()

    if date_start < freq['Date'].min():
        date_start = freq['Date'].min()
    
    freq = freq[(freq['Date'].dt.date > date_start ) & (freq['Date'].dt.date < date_end)]
    ...
    st.write('From: ',date_start, 'to: ', date_end)
